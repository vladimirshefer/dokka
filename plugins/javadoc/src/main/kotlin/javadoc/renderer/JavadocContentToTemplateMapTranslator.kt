...
internal class JavadocContentToTemplateMapTranslator(
    ...
    private inner class InnerTranslator(val contextNode: JavadocPageNode) {
        ...
        fun templateMapForIndexableNode(node: IndexableJavadocNode): TemplateMap {
            ...
            return mapOf(
                "address" to locationProvider.resolve(node.getDRI(), contextNode.sourceSets(), contextNode).formatToEndWithHtml(),
                ...
                "description" to ((node as? WithBrief)?.let { htmlForContentNodes(it.brief, contextNode).takeIf { it.isNotBlank() } } ?: "&nbsp;"),
                "originPackage" to if (origin.packageName?.isNotBlank() == true) "${origin.packageName.orEmpty()}." else "",
                "originName" to origin.classNames,
                "originAddress" to locationProvider.resolve(origin, contextNode.sourceSets(), contextNode).formatToEndWithHtml()
            )
        }
